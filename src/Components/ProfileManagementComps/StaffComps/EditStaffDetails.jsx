...
export default function EditStaffDetails() {
    ...
    const fetchAllData = async () => {
        ...
        try {
            ...
            const staff = res.data.staffinfo?.[0];

            setStaffNameEnglish(staff?.staffNameInEnglish || "");
            setStaffNameTamil(staff?.staffNameInTamil || "");
            setDateOfBirth(staff?.dateOfBirth || null);
            ...
            setWorkingStatus(studentInfo.staffWorkingStatus || "");
            setStaffExperience(studentInfo.staffExperience || "");
            setStaffIncome(studentInfo.staffIncome || "");

            const sameSchool =
                studentInfo.sameSchool?.toLowerCase() === "yes" ? "yes" : "no";
            setStudyingInSameSchool(sameSchool);

            const childrenList = studentInfo.staffStudentInfoChild || [];
            ...
            if (Array.isArray(childrenList) && childrenList.length > 0) {
                setChildren(
                    childrenList.map((c, index) => ({
                        ...
                        siblingsTwins: c.siblingsTwins || "",
                    }))
                );
                nextId.current = childrenList.length + 1;
            } else {
                setChildren([
                    {
                        id: 1,
    ...
    const handleStaffStudentSubmit = async (status) => {
        ...
        <Box sx={{ width: "100%" }}>
            ...
            <Box sx={{ maxHeight: "83vh", overflowY: "auto" }}>
                ...
                <Box sx={{ p: 2 }}>
                    ...
                    <Box pt={1}>
                        <Accordion sx={{ boxShadow: "none", border: "1px solid #ccc", borderRadius: "5px", }} defaultExpanded>
                            <AccordionSummary
                                expandIcon={<ExpandMoreIcon />}
                                aria-controls="panel1-content"
                                id="panel1-header"
                                sx={{ backgroundColor: "#fff7f7", py: 0.5, position: "relative", borderRadius: "5px" }}
                            >
                                ...
                                <AccordionDetails>
                                    <Grid container pb={2} columnSpacing={3}>
                                        ...
                                        <Box sx={{ position: "absolute", bottom: "10px", right: "10px" }}>
                                            ...
                                            <Button onClick={handleStaffStudentSubmit} sx={{ textTransform: "none", color: "#000", py: 0.2, px: 2.5, fontSize: "12px", borderRadius: "20px", backgroundColor: websiteSettings.mainColor }}>
                                                Update
                                            </Button>
                                        </Box>
                                    </Grid>
