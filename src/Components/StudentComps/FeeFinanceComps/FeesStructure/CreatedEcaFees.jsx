...
export default function CreatedEcaFees() {
  ...
  const handleEditSubmit = async () => {
    ...
    try {
      ...
      setMessage(canActDirect ? 'ECA Fee updated successfully' : 'Update request sent successfully')
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      setMessage(canActDirect ? 'ECA Fee deleted successfully' : 'Delete request sent successfully')
      setOpen(true); setColor(true); setStatus(true)
              ...
              <React.Fragment key={activity.ecaFeesID || activity.activityName}>
                <Box sx={{ display: 'flex', justifyContent: 'space-between' }}>
                  ...
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5, mt: '10px' }}>
                    ...
                    <Tooltip title={activity.isAnyStudentPaid ? 'Cannot delete — a student has already paid' : (canActDirect ? 'Delete Activity' : 'Request for Delete')} arrow>
                      <span>
                        <IconButton
          ...
          <Box sx={{ textAlign: 'center' }}>
            ...
            <Box sx={{ p: 3 }}>
              <Typography sx={{ fontSize: '17px', fontWeight: 600, color: '#111827', mb: 1 }}>
                {canActDirect ? 'Delete ECA Fee?' : 'Request for Delete?'}
              </Typography>
